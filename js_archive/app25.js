...
// If card is not already clicked, add class clicked, call showCard
// if first card is clicked, the timer starts
...
// I can use this, to call a function after the animations ends. This is maybe good, instead of timeout the the functions? It also adds and then removes the classes
$.fn.extend({
...
/* Cards Basic Functionality */
// Plays click Sound, shows the symbol of the card and animates it, after the animation ends, the function storeOpenCard is called.
function showCard(clickedCard) {
  clickAudio.play();
  $(clickedCard).addClass('open show');
  $(clickedCard).animateCss('flipInY', function() {
    storeOpenCard(clickedCard);
  });
}
/// !!! Problem: After two clicked cards it's possible to click a next card. The function preventClick is called after the animationend.
// Wrong Animation
function hideCard() {
  wrongAudio.play();
...
// Correct Animation
function correctAnimation() { // correctCard() is better???
  correctAudio.play();
...
var count = 0; // !! name this as moveCount
var starIndex = 0;
...
function stopTimer() {
  ...
  // $('#timepanel').timer().addClass('pause');
}
